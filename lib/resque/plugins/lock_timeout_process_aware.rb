  ...
  module Plugins
    ...
    module LockTimeoutProcessAware
      ...
      def acquire_lock!(*args)
        ...
        key_value  = {:lock_until=>nil,:pid=>Process.pid}
      ...
      def acquire_lock_algorithm!(lock_key,key_value)
        ...
        process_id = key_stored['pid'].to_i

        #puts "key_stored = #{key_stored}"
        #puts "lock_expiration = #{lock_expiration}"
        #puts "process_id #{process_id}"

        if (lock_expiration && lock_expiration.to_i < now) || process_dead?(process_id)
          # expired, try to acquire.
          lock_expiration = JSON.parse(Resque.redis.getset(lock_key,key_value.to_json))['lock_until']
          #puts "lock_expiration = #{lock_expiration}"
          if lock_expiration.nil? || lock_expiration.to_i < now || process_dead?(process_id)
            acquired = true
          end
        else
          # Try once more...
